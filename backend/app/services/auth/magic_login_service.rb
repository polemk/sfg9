...
module Auth
  ...
  class MagicLoginService
    ...
    def execute!
      ...
      return rate_limited_response unless can_request_code?(user, destination)
    ...
    # Marca o resultado na tentativa que ACABOU de ser criada.
    #
    # A ordenação é explícita de propósito. `.last` sem `order` usa a chave
    # primária, e aqui ela é **uuid** — ou seja, `ORDER BY id DESC` devolve uma
    # linha em ordem aleatória, não a mais recente. Três verificações
    # independentes mediram o estrago: a linha certa saía em 29% a 72,5% das
    # vezes, dependendo da amostra.
    #
    # As duas consequências, e nenhuma delas é cosmética:
    #   1. cada login BEM-SUCEDIDO deixava uma tentativa anterior marcada como
    #      falha. `suspicious_activity?` soma essas falsas falhas e **tranca o IP
    #      inteiro por 15 min** — um escritório atrás de um NAT se tranca sozinho
    #      com ~10 logins normais, e a trava se auto-alimenta;
    #   2. uma falha REAL podia ser reescrita como sucesso, o que corrompe a
    #      trilha de auditoria de segurança justamente onde ela é evidência.
